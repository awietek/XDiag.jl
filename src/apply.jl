...
function apply(
    ops::OpSum,
    bin::Block,
    vin::Vector{ComplexF64},
    bout::Block,
    vout::Vector{ComplexF64},
)
    if (length(vin) != length(bin))
        error("Input vector does not have the same size as input Block")
    elseif (length(vout) != length(bout))
...
function apply(
    ...
    if (size(min, 1) != length(bin))
        error(
            "Input matrix does not have the same number of rows as the size of the input Block",
        )
    elseif (size(mout, 1) != length(bout))
        error(
            "Output matrix does not have the same number of rows as the size of the output Block",
        )
...
function apply(
    ops::OpSum,
    bin::Block,
    min::Matrix{ComplexF64},
    bout::Block,
    mout::Matrix{ComplexF64},
)
    if (size(min, 1) != length(bin))
        error(
            "Input matrix does not have the same number of rows as the size of the input Block",
...
apply(op::Op, bin::Block, vin, bout::Block, vout) = apply(OpSum(op), bin, vin, bout, vout)

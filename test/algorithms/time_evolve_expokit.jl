...
@testset "time_evolve_expokit" begin
    ...
    psicsrC = time_evolve_expokit(csrC, r, 1.0)
    psicsrC32 = time_evolve_expokit(csrC32, r, 1.0)
    @test isapprox(psi.state, psicsr.state)
    @test isapprox(psi.state, psicsr32.state)
    @test isapprox(psi.state, psicsrC.state)
    @test isapprox(psi.state, psicsrC32.state)
    ...
    @test isapprox(psi, psicsrC)
    @test isapprox(psi, psicsrC32)
end
